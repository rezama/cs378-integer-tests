...
struct TestInteger : CppUnit::TestFixture {
    ...
    void test_multiplies_digits_1 () {
        int a[6] = {4, 3, 2, 0, 0, 0};  // 234
        const int b[] = {5, 6, 7};       // 567
		
		for(int i = 1; i < 10; i++) {cache[i].resize(3); cache[i].push_back(0);}
        fill_cache(a, a + 3);
		for(int i = 1; i < 10; i++) if(cache[i].back() == 0) cache[i].pop_back();
        cache[0].resize(1);
		a[0] = 0; a[1] = 0; a[2] = 0;
		
        const int c[] = {8, 7, 6, 2, 3, 1};  // 132678
        const int* p = multiplies_digits(a, a + 6, b, b + 3, a);
        CPPUNIT_ASSERT(p == a);
	...
	void test_multiplies_digits_2 () {
		...
		for(int i = 1; i < 10; i++) {cache[i].resize(1); cache[i].push_back(0);}
        fill_cache(a, a + 1);
		for(int i = 1; i < 10; i++) if(cache[i].back() == 0) cache[i].pop_back();
        cache[0].resize(1);
		a[0] = 0;
		
        const int c[] = {7, 6, 5}; // 567
        const int* p = multiplies_digits(a, a + 4, b, b + 3, a);
        CPPUNIT_ASSERT(p == a);
        CPPUNIT_ASSERT(std::equal(const_cast<const int*>(a), p, c));
		}
	...
	void test_multiplies_digits_3 () {
		...
		for(int i = 1; i < 10; i++) {cache[i].resize(2); cache[i].push_back(0);}
        fill_cache(a, a + 2);
		for(int i = 1; i < 10; i++) if(cache[i].back() == 0) cache[i].pop_back();
        cache[0].resize(1);
		a[0] = 0; a[1] = 0;
		
        const int c[] = {8, 8, 4, 1};  // 1488
        const int* p = multiplies_digits(a, a + 4, b, b + 2, a);
        CPPUNIT_ASSERT(p == a);
        CPPUNIT_ASSERT(std::equal(const_cast<const int*>(a), p, c));
		}	
